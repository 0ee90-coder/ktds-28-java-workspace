...
public class Collecting {
	...
	public static void printFoodTypeGroups() {
		...
		groupMap.forEach((key,value)->{		// key: FoodType , value: List<Dish>
	...
	public static void printDishNames3() {
		List<Dish>dishes = DishList.makeDishList();
		// 메뉴의 이름들을 칼로리 순으로 내림차순 하여 이름들을 "->"로 구분해서 출력한다.
		// 결과예시: 800 -> 700 -> 550 -> 530 -> 450 -> ...
		
		String names = dishes.stream()
		  .sorted((d1, d2) ->d2.getCalories() - d1.getCalories())
		  .map(d1 -> d1.getCalories()+"")
	...
	public static void printDishNames2() {
		List<Dish>dishes = DishList.makeDishList();
		//메뉴의 이름들을 칼로리 순으로 오름차순 하여 이름들을 "->"로 구분해서 출력한다.
		//결과예시: 계절과일 -> 새우 -> 쌀밥 -> 치킨 -> 연어
	...
	public static List<Integer> getOrderedList(List<Integer>intList){
		return intList.stream()						//Stream<Interger>
	...
	public static void main(String[] args) {
		...
		List<Integer>orderedList = getOrderedList(
				List.of(9,1,2,6,7,0,-1,77,8,10));
		orderedList.forEach(System.out::println);
		System.out.println("=".repeat(60));
		
		//내림차순 정렬된 리스트를 반환.
		List<Integer>descList = getDescendingOrderedList(
				List.of(9,1,2,6,7,0,-1,77,8,10));
