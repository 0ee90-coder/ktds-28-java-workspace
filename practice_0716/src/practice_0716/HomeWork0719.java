...
//	2. 정수형 변수 2개를 파라미터로 받아, 합계를 출력하는 메소드.
	public static void sumNum(int num3, int num4) {
		int plusResult = num3 + num4;
		System.out.println(plusResult);
	}
	
//	3. 정수형 변수 3개를 파라미터로 받아, 합계를 반환하는 메소드.
	public static int sumNums(int num5 , int num6 , int num7) {
		int plusResults = num5 + num6 + num7;
		return plusResults;
	}
	
//	4. 정수형 변수 2개를 파라미터로 받아, 나누기의 결과(실수)를 반환하는 메소드.
	public static float divideNum(int num8 , int num9) {
		float divideResult = num8 / num9 ;
		return divideResult;
	}
...
//	6. 실수형 변수 2개를 파라미터로 받아, 가장 큰 수만 반환하는 메소드.
	...
	public static float isMax(float num12 , float num13) {
		float max = num12;
		if (num13 > max) {
			max = num13;
		}
...
//	7. 실수형 변수 4개를 파라미터로 받아, 가장 작은 수만 반환하는 메소드.
	...
	public static float isMin(float num14 , float num15 , float num16 , float num17) {
		float min = num14;
		if (num15 < min) {
			min = num15; 
		}else if(num16 < min) {
