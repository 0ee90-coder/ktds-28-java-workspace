...
public class ListExam {
	...
	public static void main(String[] args) {
		...
		System.out.println(value1);  //IndexOutOfBoundsException 발생 이유: 0번 인덱스를 삭제했을때 1번 인덱스의 값이 0번으로 가기 때문에 1번 인덱스는 없어짐.
		...
		System.out.println(newGoods == otherGoods);		// 레퍼런스는 메모리 주소를 갖고 있어서 메모리 주소가 같은지 비교하기 때문에 false
		
		System.out.println("goods equals:" + newGoods.equals(otherGoods)); // true로 만들고 싶다
		
		Goods cloneGoods = otherGoods;
		System.out.println(cloneGoods == newGoods);		// 클론굿즈에는 아더굿즈의 메모리가 들어있다. F
		System.out.println(cloneGoods == otherGoods);	// T
		
		int n1 = 100;
		int n2 = 100;
		System.out.println(n1 == n2);					// 프리미티브 타입은 값이 같으면 같다
		...
		Goods goods0 = goodsList.get(0); // 제네릭이 Goods 이기 때문에
		System.out.println(goods0.getName());

		Goods goods1 = goodsList.get(0); // 제네릭이 Goods 이기 때문에
		...
		haveGoods = goodsList.contains(new Goods("상품11", 500));
		System.out.println(haveGoods);
