...
public class DateTime {
	...
	public static void printNowTimeWithFormat() {
		...
		// m: 분
		// s: 초
		DateTimeFormatter format = DateTimeFormatter.ofPattern("H시 m분 ss초");
	...
	public static void printNowDateTimeWithFormat() {
		LocalDateTime now = LocalDateTime.now();
		DateTimeFormatter format = DateTimeFormatter.ofPattern("yy년 MM월 dd일 EEEE H시 m분 ss초");
	...
	public static void printPeriodFromTo(LocalDate from, LocalDate to) {
		...
		System.out.println("%s 부터 %s 까지 %d년 %d개월 %d일 지났습니다."
				.formatted(from, to, period.getYears(), period.getMonths(), period.getDays()));
	}
	...
	public static void printIsAfter(LocalDate date) {
		...
		System.out.println(" %s는 %s 보다 ".formatted(now, date));
		if(isFuture) {
...
//		setAndPrintTime(25, 8, 35);		java.time.DateTimeException
		...
		setAndPrintTime("11:01:10");
		setAndPrintDateTime("2000-10-10T23:11:56");
		...
		setAndPrintTimeWithFormat("11:55:33", "HH:mm:ss");
		setAndPrintTimeWithFormat("11시 55분 33초", "HH시 mm분 ss초");
		setAndPrintTimeWithFormat("11시 55분", "HH시 mm분");
		setAndPrintTimeWithFormat("11.55", "HH.mm");
		
		setAndPrintDateTimeWithFormat("2026년 08월 04일 11시 55분 33초", "yyyy년 MM월 dd일 HH시 mm분 ss초");
		setAndPrintDateTimeWithFormat("26년08월04일 11시55분", "yy년MM월dd일 HH시mm분");
//	    setAndPrintDateTimeWithFormat("'26.08.04 11:55", "'yy.MM.dd HH:mm");
	    
	    plusYearsAndPrintDateFromNow();
	    plusDaysAndPrintDateFromNow();
	    
	    printPeriodFromTo(LocalDate.parse("1985-11-16"), LocalDate.now());
	    printBetweenDaysFromTo(LocalDate.parse("1985-11-16"), LocalDate.now());
	    printBetweenDaysFromTo(LocalDate.now(), LocalDate.parse("1985-11-16"));
