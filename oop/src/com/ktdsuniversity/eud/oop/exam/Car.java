package com.ktdsuniversity.eud.oop.exam;

public class Car {
	
	//멤버변수
	//클래스의 인스턴스가 독립적으로 가지고 있는 정보들
	boolean isEngineStart;
	int speed;
	public int zero100Seconds;
	
	public Car(int zero100Seconds) {
		System.out.println("생성자를 호출했습니다.");
		System.out.println(this); //new car
		this.pressEngineStartButton();
		this.zero100Seconds = zero100Seconds; // 인스턴스의 멤버변수들을 호출할 때는 this 붙이기
	}
	
	// 인스턴스 메소드 (이하 메소드) 정의.
	// 엔진 켜기/끄기
	public void pressEngineStartButton() {
		// 이 메소드를 호출하는 인스턴스의 isEngineStart가 false => true, true => false
	//	isEngineStart = !isEngineStart; 아래의 코드와 똑같다
		
		if (isEngineStart) {
			// 시동이 켜져있다면 끈다.
			isEngineStart = false;
			speed = 0;
		} else {
			// 시동이 꺼져있다면 켠다.
			isEngineStart = true;
			speed = 10;
		}
	}
	
	// 가속페달을 밟는다.
	// 페달을 밟는 압력만큼 속도가 증가한다.
	public void pressGasolinPedal(int pressure) {
		if(isEngineStart) {
		speed += pressure;
		}
	}	
	
	// 브레이크 페달을 밟는다.
	// 페달을 밟는 압력만큼 속도가 감속한다.
	
	public void pressBrakePedal(int pressure) {
		if(isEngineStart) {
			speed -= pressure;
		}
	}
	
	@Override
	public String toString() {
		//출력의 형태를 정의.
		
		return this.zero100Seconds + "";
        		
	
	}
	

	
	public static void main(String[] args) {
		
		// Car 클래스를 이용해 다음을 생성.
		
		// Car 타입의 kona 인스턴스를 생성.
		Car kona = new Car(8);
		
		// Car 타입의 carnival 인스턴스를 생성.
		Car carnival = new Car(7);
		
		System.out.println(kona); // Type@HashData
		System.out.println(carnival); // Type@HashData
		
		String name = "fdfdfdf";  // name의 타입은 string 따라서 변수가 아니라 인스턴스이다
		System.out.println(name); // fdfdfdf , System은 클래스 , out은 변수가 아니라 인스턴스 . 이 있기에 / 
		
		StringBuffer buffer = new StringBuffer();
		buffer.append("asdfasdfsda");
		System.out.println(buffer);
		
		// kona의 시동을 켠다.
		kona.pressEngineStartButton();
		
		// 브레이크 페달을 몇차레 밟는다.
		kona.pressBrakePedal(3);
		kona.pressBrakePedal(3);
		
		// 가속 페달을 3회 강하게 밟는다.
		kona.pressGasolinPedal(150);
		kona.pressGasolinPedal(150);
		kona.pressGasolinPedal(150);
		
		// 브레이크 페달을 아주 강하게 밟는다.
		kona.pressBrakePedal(300);
		
		// kona의 시동을 끈다.
		kona.pressEngineStartButton();
		
		// carnival의 시동을 켠다.
		carnival.pressEngineStartButton();
		
		// 브레이크 페달을 한번 밟는다.
		carnival.pressBrakePedal(2);
		
		// 가속 페달을 2회 강하게 밟는다.
		carnival.pressGasolinPedal(200);
		carnival.pressGasolinPedal(200);
		
		// 브레이크 페달을 강하게 밟는다.
	    carnival.pressBrakePedal(300);
	    
	    // carnival의 시동을 끈다.
	    carnival.pressEngineStartButton();
	    
	    
		
		// 인스턴스는 자신의 멤버변수의 값을 할당하거나 출력할 수 있다.
//		kona.isEngineStart = true; // 위에 클래스의 인스턴스가 독립적으로 가지고 있는 정보들이라 할당 할 수 있다.
//		kona.speed = 100;
//		인스턴스 메소드가 영향을 받거나 영향을 준다.
//		
//		carnival.isEngineStart = false;
//		carnival.speed = 70;
		
		System.out.println(kona.isEngineStart);
		System.out.println(carnival.speed);
		
		System.out.println(carnival.isEngineStart);
		System.out.println(carnival.speed);
		
	}
	
}
