...
public class TestCar {
	...
	public static void startTurbeEngine(Vehicle car) {
		if( car instanceof SportsCar sportscar) {
			sportscar.startTurbeEngine();
		}
	}
	...
	public static void ejectPod(Vehicle car) {
		// car로 전달된 인스턴스의 타입이 BatMobile 이라면
		// ejectBatPod을 실행시켜라.
	...
	public static void main (String[] args) {
		...
		vehicles[0] = new SportsCar("스포츠 카1");
		vehicles[0] = new Ev("전기차2", 50f);
		vehicles[0] = new BatMobile("배트카1");
		vehicles[0] = new BatMobile("배트카2");
		
		for(int i = 0; i < vehicles.length; i++) {
			startEngine(vehicles[i]);
			checkBattery(vehicles[i]);
			startTurbeEngine(vehicles[i]);
			ejectPod(vehicles[i]);
		}
	}
}
